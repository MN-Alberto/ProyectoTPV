...
// ✅ SOLUCION MEJORADA: Mostrar ventas de sesión activa si existe, sino del día natural
if ($sesionCaja) {
    // Hay sesión de caja ABIERTA: Mostrar SOLAMENTE ventas de ESTA sesión
    $stmt = $pdo->prepare("
        SELECT COALESCE(SUM(total), 0) FROM ventas 
        WHERE idSesionCaja = ? 
        AND estado = 'completada'
    ");
    $stmt->execute([$sesionCaja->getId()]);
    $totalVentasHoy = $stmt->fetchColumn();

    // Mantener título original de sesión actual
    $tituloVentas = "Ganancias (Sesión Actual)";
} else {
    // Caja cerrada: Mostrar ventas del día natural entero
